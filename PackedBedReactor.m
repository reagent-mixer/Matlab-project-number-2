...
%Packed Bed Reactor V.4
%Assumptions
%1.Steady State.
%2.Uniform Particle size and shape.
%% Inputa data/parameters

% Operating conditions
alpha = 9.8e-5;   % pressure drop coefficient [1/kg]
FAin  = 100;       % inlet molar flow rate of A [mol/min]
Pin   = 40;       % pressure [atm]
T     = 640;      % temperature [C]
rhoB  = 400;      % bed density [kg/m3]
xAin  = 0.45;     % inlet molar fraction of A
xBin  = 0.30;     % inlet molar fraction of B
...
%% Part 1: PBR with pressure drop(Graph Drop)
...
%% Part 2: PBR without pressure drop(Graph No Drop)
...
%% ODE system 1: PBR with pressure drop(Calculates Drop)
function dX_over_dW = withpressuredrop(W,X, k,Kc,Ka,PAin,FAin,alpha)
    ...
    disp( sprintf('dX_over_dW'));
    disp(dX_over_dW);
    
  end


%% ODE system 2: PBR without pressure drop(No Pressure Drop Calculation)
