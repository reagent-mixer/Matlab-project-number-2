close all; clear variables;
%Continuous Stirred Tank Reactor V.3
%Assumptions
%1.Perfect Mixing (rA is same throughout reactor).
%2.Steady State.
%3.Constant Fluid Density.


%% Input data/Parameters

CA0 = 1.0;    % initial concentration [mol/l]
k1  = 0.5;    % kinetic constant [1/s]
k2  = 2;    % kinetic constant [1/s]
Q   = 10;   % Feed Flow rate [l/m]
V   = 2;  % volume of CSTR[l]

%% Optimal values (analytical solutions)

% Optimal residence time
tOpt = 1/sqrt(k1*k2);

% Concentrations
CAOpt = CA0/(1+k1*tOpt);
CDOpt = CA0*k1*tOpt/(1+k1*tOpt)/(1+k2*tOpt);
CUOpt = CA0 - CAOpt - CDOpt;

% Conversion
XOpt = (CA0-CAOpt)/CA0;

% Formation rates
RAOpt = -k1*CAOpt;
RDOpt = k1*CAOpt - k2*CDOpt;
RCOpt = k2*CDOpt;

% Selectivity
SOpt = RDOpt/RCOpt;
StildeOpt = CDOpt/CUOpt;

% Fractional Yield
YOpt = RDOpt/(-RAOpt);
YtildeOpt = CDOpt/(CA0-CAOpt);

% Assumed Tau
Tau=(Q/60)/V;
disp( sprintf('Assumed Tau'));
disp(Tau);

% Calculated Tau

tau=(CA0-CAOpt)/k1*CAOpt^2;
disp( sprintf('Actual Tau'));
disp(tau);

% Rate of Reaction
rA=(CA0-CAOpt)/tau*CAOpt^2;
disp( sprintf('Rate of Reaction'));
disp(rA);


%% Profiles (analytical solutions)

t = linspace(0,5*tOpt,200);

CA = CA0./(1+k1*t);
CD = CA0*k1*t./(1+k1*t)./(1+k2*t);
CU = CA0 - CA - CD;

XA = (CA0-CA)/CA0;

RA = -k1*CA;
RD = k1*CA - k2*CD;
RC = k2*CD;

S = RD./(RC+1e-6);
Stilde = CD./(CU+1e-6);

Y = RD./(-RA);
Ytilde = CD./(CA0-CA);


%% Plotting solution(Graphs of Reactor internal environment)

% Concentrations vs time
subplot(2,3,1);
hold all;
plot(t,CA,'LineWidth',2);
plot(t,CD,'LineWidth',2); 
plot(t,CU,'LineWidth',2);
title('Concentrations');    legend('A', 'D', 'U');
xlabel('time [m]'); ylabel('concentrations [mol/l]');
xlim([0 3*tOpt]);

% Concentrations vs Conversion
subplot(2,3,2);
hold all;
plot(XA,CA,'LineWidth',2);
plot(XA,CD,'LineWidth',2);
plot(XA,CU,'LineWidth',2);
title('Concentrations'); legend('A', 'D', 'U');
xlabel('X_A'); ylabel('concentrations [mol/l]');

% Fractional yield vs Conversion
subplot(2,3,3);
hold all;
plot(XA,Ytilde,'LineWidth',2);
title('Yield');
xlabel('X_A'); ylabel('yield');

% Selectivity vs Conversion
subplot(2,3,4);
hold all;
plot(XA(2:end),S(2:end), 'LineWidth',2);
plot(XA(2:end), Stilde(2:end), 'LineWidth',2);
title('Selectivity'); legend('instantaneous', 'overall');
xlabel('X_A'); ylabel('selectivity');

% Conversion vs time
subplot(2,3,5);
hold all;
plot(t,XA,'LineWidth',2);
title('Conversion vs Time');    legend('A', 'D', 'U');
xlabel('time [m]'); ylabel('conversion');
xlim([0 3*tOpt]);
